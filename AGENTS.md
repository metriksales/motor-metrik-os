...
9. **Toda consulta roda dentro de um contexto de conta.** A aplicação conecta como `metrik_app`, que não é dono das tabelas: sem declarar a conta, as políticas de RLS devolvem **zero linhas** — não erro, zero linhas. Use `comConta(orgId, …)` para trabalho de conta e `comPessoa(userId, …)` para o que atravessa contas (seletor, convite). O que legitimamente escapa do RLS são funções `SECURITY DEFINER` nomeadas, e a lista inteira cabe num fôlego: as que resolvem sessão, token e segredo de entrada por hash, mais `tem_convite_pendente`, `aceitar_convites_do_email` e `fundar_conta`. **Não acrescente uma sem dizer por quê.**
