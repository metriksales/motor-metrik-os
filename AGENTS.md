...
9. **Toda consulta roda dentro de um contexto de conta.** A aplicação conecta como `metrik_app`, que não é dono das tabelas: sem declarar a conta, as políticas de RLS devolvem **zero linhas** — não erro, zero linhas. Use `comConta(orgId, …)` para trabalho de conta e `comPessoa(userId, …)` para o que atravessa contas (seletor, convite). O que legitimamente escapa do RLS são funções `SECURITY DEFINER` nomeadas, e a lista inteira cabe numa olhada — é para isso que ela existe:

   - **resolver por segredo** (`resolver_sessao_por_hash`, `resolver_token_por_hash`, `resolver_entrada_por_hash`, `papel_na_conta`, `marcar_uso_do_token`, `tocar_sessao`, `encerrar_sessao`): descobrem a conta a PARTIR de um hash, então não há contexto a declarar — é o contexto que elas produzem.
   - **antes de haver pessoa** (`pessoa_por_email`, `existe_alguma_pessoa`, `garantir_pessoa`, `pedidos_recentes`, `guardar_codigo`, `codigo_vigente`, `queimar_tentativa`, `marcar_codigo_usado`, `tem_convite_pendente`): o login acontece antes de qualquer contexto. `login_codes` não tem política NENHUMA: só se toca nela por aqui.
   - **escrita privilegiada** (`aceitar_convites_do_email`, `aceitar_convite_por_hash`, `fundar_conta`): vinculam pessoa a conta sem estar dentro dela. São privilégio, e privilégio é função nomeada em vez de política frouxa — "quem tem o token pode se vincular" como regra geral é exatamente o buraco que a S-011 fechou.

   Nenhuma delas lista nada: todas exigem o hash, o e-mail ou o id exato em mãos. **Não acrescente uma sem dizer por quê.**
10. **Credencial de cliente só sai por `usarCredencial`.** Não existe ação de API que devolva o valor de um segredo, e não crie uma. Guardar, listar e revogar nunca retornam o valor — nem logo depois de recebê-lo. A dica (`…1234`) é o que a tela mostra.
11. **Cuidado ao misturar o bundle com a fonte em teste.** `api/_bundled/control.mjs` carrega a PRÓPRIA cópia de `@motor/db`, com outro `AsyncLocalStorage` e outro pool. Embrulhar uma chamada do bundle com o `comConta` do módulo TypeScript não transporta contexto nenhum — e o sintoma (insert recusado) não diz isso.
12. **Migração só roda na produção.** `migrar.mjs` para se `VERCEL_ENV` não for `production`: o preview usa a **mesma** `DATABASE_URL` e sai **antes** do deploy de produção, então sem a trava uma migração meio pronta de uma branch qualquer chega ao banco real antes de qualquer revisão. Migração nova se exercita na CI, contra o Postgres de teste.
